...
- `src/services/blocking.ts` — Screen Time boundary, built on `react-native-device-activity` (needs the Family Controls entitlement and a dev client; every call no-ops in Expo Go, where the mock app list stands in for Apple's picker). Apple returns an opaque selection, so everything picked is shielded as one unit under `SELECTION_ID`. `blockSelection` is additive, so `applyShields` calls `resetBlocks` first. Unlock windows are enforced by a DeviceActivity usage threshold, which counts time in the app rather than wall-clock.
...
- Shield return path: Apple blocks app extensions from launching apps, so the library's `openApp` action (`NSExtensionContext().open` of a hardcoded `device-activity://`, which `app.json` registers as a second scheme) does nothing on current iOS — `close` just dismisses the shield, `defer` does nothing. The dependable path is the shield's `sendNotification` action; tapping it routes to the gate via `onUnlockNotificationTap` in `RootNavigator`. Notification permission is therefore load-bearing and is requested in `AppPickerScreen`.
