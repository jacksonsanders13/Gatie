...
- `extension/` — Chrome MV3 extension (Vite + React, own package.json/node_modules; Metro and the root tsconfig ignore it). Reuses `src/state/{actions,selectors,types}.ts` directly — those files must stay free of React Native imports. Blocking = declarativeNetRequest dynamic redirect rules to `gate.html` (`extension/src/rules.ts`), state in `chrome.storage.local`, relock tabs via `chrome.alarms` in `background.ts`. No web billing yet, so `isPro` is forced true. Build: `npm run build --prefix extension`, load `extension/dist` unpacked.
